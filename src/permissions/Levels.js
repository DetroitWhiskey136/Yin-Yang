...
const permLevels = [
  ...
  // Level 5 ~ Moderator or higher has access
  {
    ...
    name: 'moderator',
    check: (message, bot) => {
      const role = bot.database.fn.guild.get(message.guild.id).moderator.modrole;
    ...
    checkMember: (member, bot) => {
      const role = bot.database.fn.guild.get(member.guild.id).moderator.modrole;
      if (!role) return false;
      try {
  ...
  // Level 6 ~ Admin or higher has access
  {
    ...
    name: 'administrator',
    check: (message, bot) => {
      const role = bot.database.fn.guild.get(message.guild.id).admin.adminrole;
      if (!role) return false;
      try {
    ...
    checkMember: (member, bot) => {
      const role = bot.database.fn.guild.get(member.guild.id).admin.adminrole;
      if (!role) return false;
      try {
  ...
  // Level 8 ~ Bot Support or higher has access
  {
    level: 8,
    name: 'botsupport',
    check: (message, bot) => bot.database.fn.settings.support.includes(message.author.id),
    checkMember: (member, bot) => bot.database.fn.settings.support.includes(member.id),
  },
  // Level 9 ~ Bot Admin or higher has access
  {
    level: 9,
    name: 'botadmin',
    check: (message, bot) => bot.database.fn.settings.admins.includes(message.author.id),
    checkMember: (member, bot) => bot.database.fn.settings.admins.includes(member.id),
  },
  // Level 10 ~ Bot owner has access
  /* {
    level: 10,
    name: 'botowner',
    check: (message, bot) => bot.appInfo.owner.members.has(message.author.id),
    checkMember: (member, bot) => bot.appInfo.owner.members.has(member.id),
  }, */
];
